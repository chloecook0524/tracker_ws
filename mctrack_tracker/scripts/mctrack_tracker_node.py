...
class KalmanTrackedObject:
    ...
    def predict(self, dt, ego_vel=0.0, ego_yaw_rate=0.0, ego_yaw=0.0):
        ...
        if len(self.bboxes) > 30:
            self.bboxes.pop(0)
        # 상태 전이 수행
        self.update_status(matched_score=0.0)    
    ...
    def update(self, detection, dt, matched_score):
        ...
        self.traj_length += 1
        self.reproj_bbox = detection.get('reproj_bbox')
        matched_score = detection.get("confidence", 0.5)  # 또는 다른 이름
        self.confidence = matched_score  # 내부 저장도 함께
        ...
        if len(self.bboxes) > 30:
            self.bboxes.pop(0)
        self.update_status(matched_score)    

    def update_status(self, matched_score):
        config = CLASS_CONFIG.get(self.label, {})
        max_predict_len = config.get("max_predict_len", 20)
        confirmed_det_score = config.get("confirmed_det_score", 0.7)
        confirmed_match_score = config.get("confirmed_match_score", 0.3)

        # INIT → CONFIRMED
        if self.status_flag == TrackState.INITIALIZATION:
            if self.traj_length > self.confirm_threshold or (
                matched_score > confirmed_match_score and
                getattr(self, "confidence", 0.5) > confirmed_det_score
            ):
                self.status_flag = TrackState.CONFIRMED

        # CONFIRMED → OBSCURED
        if self.status_flag == TrackState.CONFIRMED:
            if self.missed_count > self.max_missed:
                self.status_flag = TrackState.OBSCURED

        # OBSCURED → DEAD
        if self.status_flag == TrackState.OBSCURED:
            if self.missed_count > self.max_missed + max_predict_len:
