...
def velocity_alignment_bonus(track_vel, det_vel):
    v1 = np.array(track_vel)
    v2 = np.array(det_vel)
...
def hungarian_iou_matching(tracks, detections, use_hybrid_cost=False, dt=0.1):
    ...
    for i, track in enumerate(tracks):
        for j, det in enumerate(detections):
            ...
            # ✅ velocity alignment 보정 추가
            v_align = velocity_alignment_bonus(track.pose_state[2:4], det["velocity"])
            cost *= (1.0 - 0.15 * v_align)
...
class MCTrackTrackerNode:
    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            if rospy.has_param("/logger_ready"):
                rospy.loginfo("[TrackerNode] Logger ready detected.")
                break
            rospy.logwarn("[TrackerNode] Waiting for /logger_ready...")
            rospy.sleep(0.5)

        # 3) Baseversion JSON에서 토큰 정렬
        try:
            with open(BASE_DET_JSON, "r") as f:
                det_json = json.load(f)
            token_set = set()
            for seq_frames in det_json.values():
                for frame in seq_frames:
                    token = frame.get("cur_sample_token")
                    if token:
                        token_set.add(token)
            self.sorted_tokens = sorted(token_set)  # time순 정렬 보장 안 될 수 있음
            rospy.loginfo(f"[TrackerNode] Loaded {len(self.sorted_tokens)} tokens from base detection JSON.")
        except Exception as e:
            rospy.logerr(f"[TrackerNode] Failed to load BASE_DET_JSON: {e}")
            self.sorted_tokens = []
        ...
        self.tracker = KalmanMultiObjectTracker(
            use_hungarian=True,
            use_hybrid_cost=True
        )

        
        # 6) 퍼블리셔 & 구독자
        self.tracking_pub = rospy.Publisher("/tracking/objects", PfGMFATrackArray, queue_size=100)
        ...
        rospy.set_param("/tracker_ready", True)
    ...
    def tracking_publish_callback(self, event):
        ...
        ta.header.frame_id = token  # ✅ GT 매핑용 토큰 저장
    ...
    def detection_callback(self, msg):
        try:
            ...
            dt = 0.0 if self.last_time_stamp is None else (msg.header.stamp - self.last_time_stamp).to_sec()
            ...
            class_min_confidence = {
                ...
                3: 0.16,  # bus
                4: 0.17,  # trailer
                2: 0.00,  # truck (no filtering)
            }
            detections = []
            for i, obj in enumerate(msg.objects):
                label = obj.label
                if label in VALID_CLASSES and obj.score >= class_min_confidence.get(label, 0.0):
                    detections.append({
                        "id":         i,
                        "position":   [obj.pos_x, obj.pos_y, obj.pos_z],
                        "yaw":        obj.yaw,
                        "size":       obj.size,  # 이미 [w, l, h] 순서의 float32[3]이므로 그대로 사용
                        "type":       obj.label,
            ...
            token = msg.header.frame_id.strip()

            ta = PfGMFATrackArray()
            ta.header.stamp = msg.header.stamp
            ta.header.frame_id = token
            for t in tracks:
                m = PfGMFATrack()
                m.pos_x = t["x"]
                m.pos_y = t["y"]
            ...
            if hasattr(self, "vis_pub"):
                ...
                for tid in deleted_ids:
                    for ns, offset in [("track_meshes", 0), ("track_ids", 1000), ("track_arrows", 2000)]:
                        m = Marker()
                        m.header = vis_header
                        m.ns = ns
                        m.id = tid + offset
                ...
                for t in tracks:
                    self.marker_array.markers.append(create_single_track_marker(t, vis_header, t["id"]))
                    text_m = create_text_marker(t, vis_header, 1000 + t["id"])
                    if text_m: self.marker_array.markers.append(text_m)
                    self.marker_array.markers.append(create_arrow_marker(t, vis_header, 2000 + t["id"]))

                self.marker_array.markers.append(create_ego_marker(vis_header.stamp))
                self.vis_pub.publish(self.marker_array)
