...
class GeneralizedLSSFPN(BaseModule):
    ...
    def __init__(
        ...
        for i in range(self.start_level, self.backbone_end_level):
            l_conv = ConvModule(
                in_channels[i] +
                (in_channels[i + 1] if i == self.backbone_end_level - 1 else out_channels),
                out_channels,
                1,
                conv_cfg=conv_cfg,
...
@MODELS.register_module()
class SpecializedLSSFPN(BaseModule):
    ...
    def __init__(
        ...
        for i in range(self.start_level, self.backbone_end_level):
            l_conv = ConvModule(
                in_channels[i] +
                (0 if i == self.backbone_end_level - 1 else out_channels),
                out_channels,
                1,
                conv_cfg=conv_cfg,
                norm_cfg=norm_cfg if not self.no_norm_on_lateral else None,
    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels-1, -1, -1):
            ...
            if i != used_backbone_levels-1:
                laterals[i] = torch.cat([x, laterals[i+1]], dim=1)
            else:
                laterals[i] = x
            laterals[i] = self.lateral_convs[i](laterals[i])
            laterals[i] = self.fpn_convs[i](laterals[i])
