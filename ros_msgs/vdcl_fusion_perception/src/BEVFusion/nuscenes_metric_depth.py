...
from .eval.config import config_factory
...
class NuScenesMetricDepth(BaseMetric):
    ...
    def __init__(self,
                 ...
                 load_json: Optional[str] = None) -> None:
        self.default_prefix = 'NuScenes metric depth'
        super(NuScenesMetricDepth, self).__init__(
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            if 'pred_pts_seg' in data_sample:
                ...
                for attr_name in pred_pts_seg:
                    pred_pts_seg[attr_name] = pred_pts_seg[attr_name].to('cpu')
                result['pred_pts_seg'] = pred_pts_seg

                gt_pts_seg = data_sample['gt_pts_seg']
                for attr_name in gt_pts_seg:
                    gt_pts_seg[attr_name] = gt_pts_seg[attr_name].to('cpu')
                result['gt_pts_seg'] = gt_pts_seg
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        print('load json:', self.load_json)
        if self.load_json:
            print('loading json')
            result_dict = {}
            json_path = osp.join("./test_result", self.load_json)
            # for folder_name in os.listdir(json_path):
            folder_path = osp.join(json_path, 'pred_instances_3d', 'results_nusc.json')
            result_dict['pred_instances_3d'] = folder_path
            tmp_dir = None
            print(result_dict)
    ...
    def nus_evaluate(self,
        ...
        for name in result_dict:
            ...
            ret_dict = self._evaluate_single(
                ...
                output_dir=self.jsonfile_prefix)
    ...
    def _evaluate_single(
            ...
            output_dir=None) -> Dict[str, float]:
        ...
        # from .eval.evaluate_static_dist_th import NuScenesEvalDepth
        from .eval.evaluate import NuScenesEvalDepth
        ...
        # metrics = mmengine.load(osp.join(output_dir, 'metrics_summary_0to30.json'))
        metrics = mmengine.load(osp.join(output_dir, 'metrics_summary_0to100.json'))
        detail = dict()
