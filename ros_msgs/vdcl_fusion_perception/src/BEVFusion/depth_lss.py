...
class BaseDepthTransform(BaseViewTransform):
    ...
    def forward(
        ...
        **kwargs,
        ...
        x, x_depth = self.get_cam_feats(img, depth)
        x = self.bev_pool(geom, x)
        ...
        if self.vis:
            ...
            bev_img = torch.linalg.norm(x[0].clone().detach().cpu(), dim=0)
            # bev_img_max = bev_img.max()
            # bev_img = torch.clamp(bev_img,max=bev_img_max*0.005)
            # bev_img_mean = bev_img.mean()
            # bev_img_std = bev_img.std()
            # bev_img = (bev_img - bev_img_mean) / bev_img_std
            bev_img += bev_img.mean()
            im = ax.imshow(bev_img, cmap='viridis',norm=colors.LogNorm(vmin=bev_img[bev_img>0].min(), vmax=bev_img.max()))
            ax.set_xticks(list(range(6,bev_img.shape[1],50)))
            ax.set_yticks(list(range(6,bev_img.shape[0],50)))
            # ax.grid(True, color='white', linewidth=0.1, linestyle='-')
            ...
            bev_img = torch.linalg.norm(x_depth[0].clone().detach().cpu(), dim=0)
            # bev_img_max = bev_img.max()
            # bev_img = torch.clamp(bev_img,max=bev_img_max*0.005)
            # bev_img_mean = bev_img.mean()
            # bev_img_std = bev_img.std()
            # bev_img = (bev_img - bev_img_mean) / bev_img_std
            # print("bev_img_mean", bev_img_mean)
            # print("bev_img_std", bev_img_std)
            # bev_img = torch.log1p(bev_img)
            bev_img += bev_img.mean()
            im = ax.imshow(bev_img, cmap='viridis',norm=colors.LogNorm(vmin=bev_img[bev_img>0].min(), vmax=bev_img.max()))
            ax.set_xticks(list(range(6,bev_img.shape[1],50)))
            ax.set_yticks(list(range(6,bev_img.shape[0],50)))
