...
def accumulate(gt_boxes: EvalBoxes,
    ...
    for ind in sortind:
        ...
        dist_th = pred_box.ego_dist / 12.5
    ...
    # Calculate precision and recall with confidence threshold.
    conf_threshold = np.arange(0, 0.5, 0.01)
    total_rec = {0:0}
    total_prec = {0:0}
    total_f1_score = {0:0}
    for threshold in conf_threshold:
        ...
        if np.sum(mask) == 0:
            continue
        prec_masked = tp[mask] / (tp[mask] + fp[mask])
        rec_masked = tp[mask] / float(npos)

        total_rec[threshold] = rec_masked[-1]
        total_prec[threshold] = prec_masked[-1]
        total_f1_score[threshold] = 2 * (prec_masked[-1] * rec_masked[-1]) / (prec_masked[-1] + rec_masked[-1])
