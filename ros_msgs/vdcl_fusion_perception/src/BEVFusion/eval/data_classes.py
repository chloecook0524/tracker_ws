...
class DetectionMetricData(MetricData):
    ...
    def __init__(self,
        ...
        self.total_f1_score = total_f1_score
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(recall=np.array(content['recall']),
                   ...
                   total_recall=content['total_recall'],
                   total_precision=content['total_precision'],
                   total_f1_score=content['total_f1_score'])
    ...
    def random_md(cls):
        ...
        return cls(recall=np.linspace(0, 1, cls.nelem),
                   ...
                   total_recall=np.random.random(),
                   total_precision=np.random.random(),
                   total_f1_score=np.random.random())
...
class DetectionMetrics:
    ...
    def __init__(self, cfg: DetectionConfig):
        ...
        self._label_recalls = defaultdict(lambda: defaultdict(float))
        self._label_precisions = defaultdict(lambda: defaultdict(float))
        self._label_f1_scores = defaultdict(lambda: defaultdict(float))
        self.eval_time = None
    ...
    def add_label_recall(self, detection_name: str, dist_th: float, recall: dict) -> None:
        self._label_recalls[detection_name][dist_th] = recall

    def add_label_precision(self, detection_name: str, dist_th: float, precision: dict) -> None:
        self._label_precisions[detection_name][dist_th] = precision

    def add_label_f1_score(self, detection_name: str, dist_th: float, f1_score: dict) -> None:
        self._label_f1_scores[detection_name][dist_th] = f1_score
    ...
    def mean_recall_precision_f1score(self):
        detection_name_recall = []
        detection_name_precision = []
        detection_name_f1_score = []
        for detection_name, f1_scores in self._label_f1_scores.items():
            dist_recall = []
            dist_precision = []
            dist_f1_score = []
            for dist_th, f1_score in f1_scores.items():
                if len(f1_score) == 0:
                    dist_recall.append(0)
                    dist_precision.append(0)
                    dist_f1_score.append(0)
                else:
                    max_threshold, max_f1_score = max(f1_score.items(), key=lambda x: x[1])
                    # max_threshold = 0.1
                    # max_f1_score = f1_score[max_threshold]
                    dist_recall.append(self._label_recalls[detection_name][dist_th][max_threshold])
                    dist_precision.append(self._label_precisions[detection_name][dist_th][max_threshold])
                    dist_f1_score.append(max_f1_score)
            detection_name_recall.append(np.mean(dist_recall))
            detection_name_precision.append(np.mean(dist_precision))
            detection_name_f1_score.append(np.mean(dist_f1_score))
        return np.mean(detection_name_recall), np.mean(detection_name_precision), np.mean(detection_name_f1_score)


    def serialize(self):
        mean_recall, mean_precision, mean_f1_score = self.mean_recall_precision_f1score()
        return {
            ...
            'label_precisions': self._label_precisions,
            'label_f1_scores': self._label_f1_scores,
    ...
    def deserialize(cls, content: dict):
        ...
        for detection_name, label_recalls in content['label_recalls'].items():
            for dist_th, recall in label_recalls.items():
                metrics.add_label_recall(detection_name=detection_name, dist_th=float(dist_th), recall=recall)
        
        for detection_name, label_precisions in content['label_precisions'].items():
            for dist_th, precision in label_precisions.items():
                metrics.add_label_precision(detection_name=detection_name, dist_th=float(dist_th), precision=precision)
            
        for detection_name, label_f1_scores in content['label_f1_scores'].items():
            for dist_th, f1_score in label_f1_scores.items():
                metrics.add_label_f1_score(detection_name=detection_name, dist_th=float(dist_th), f1_score=f1_score)
