...
from .data_classes import DetectionBox
from nuscenes.eval.detection.utils import category_to_detection_name
from nuscenes.eval.tracking.data_classes import TrackingBox
from nuscenes.utils.data_classes import Box
from nuscenes.utils.geometry_utils import points_in_box
from nuscenes.utils.splits_carla_bus import create_splits_scenes
    ...
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        if is_pred:
            eval_boxes.boxes[sample_token] = [box for box in eval_boxes[sample_token] if
                                              box.ego_dist < max_dist[box.__getattribute__(class_field)]+4]
        else:
            eval_boxes.boxes[sample_token] = [box for box in eval_boxes[sample_token] if
...
def filter_eval_boxes_min_max(nusc: NuScenes,
                      eval_boxes: EvalBoxes,
    ...
    eval_boxes = deepcopy(eval_boxes)
    ...
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        if is_pred:
            eval_boxes.boxes[sample_token] = [box for box in eval_boxes[sample_token] if
                                            min_dist[box.__getattribute__(class_field)]-4 <= box.ego_dist < max_dist[box.__getattribute__(class_field)]+4]
        else:
            eval_boxes.boxes[sample_token] = [box for box in eval_boxes[sample_token] if
                                          min_dist[box.__getattribute__(class_field)] <= box.ego_dist < max_dist[box.__getattribute__(class_field)]]
        dist_filter += len(eval_boxes[sample_token])
