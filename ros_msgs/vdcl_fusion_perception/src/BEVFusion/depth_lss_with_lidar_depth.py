...
class DepthLSSTransform_with_lidar_depth(BaseDepthTransform):
    ...
    def get_cam_feats(self, x, d, pe, metas=None):
        ...
        img_features = x.view(B * N, C, fH, fW)
        ...
        #     d = d.float()
        #     self.dtransform = self.dtransform.float()
        ...
        # print("x", x.shape)
        x = torch.cat([d_transform, img_features], dim=1)
        depth_net_img_feature = self.depthnet(x)

        # depth = x[:, :self.D].softmax(dim=1)

        # depth_loss = self.wasserstein_loss(depth, depth_from_lidar)
        
        x = depth.unsqueeze(1) * depth_net_img_feature.unsqueeze(2)
        ...
        if self.vis:
            ...
            img_features = img_features.clone().detach().cpu()
            depth_net_img_feature = depth_net_img_feature.clone().detach().cpu()
            ...
            fig, ax = plt.subplots(2, 3, figsize=(12, 6))
            for i in range(6):
                img_feature = depth_net_img_feature[i].max(0)[0]
                im = ax[i//3, i%3].imshow(img_feature, cmap='viridis')
                ax[i//3, i%3].set_title('Image '+str(i))
            fig.suptitle(f'Cam Feature')
            fig.colorbar(im, ax=ax,location='right',label='Feature')
            plt.savefig(f'test_data/depth_net_img_feature{img_num}.png')
            plt.close()
    ...
    def downsample_depth(self, depth_map, scale, D):
        ...
        D_channels.index_put_(
            ...
            (1 - weights).to(D_channels.dtype),
            accumulate=True
        )
        D_channels.index_put_(
            (batch_indices, bin_indices + 1, downsampled_y, downsampled_x),
            weights.to(D_channels.dtype),
            accumulate=True
        )
