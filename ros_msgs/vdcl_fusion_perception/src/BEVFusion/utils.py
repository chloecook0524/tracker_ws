...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, train_cfg):
        ...
        try:
            ...
            print("[WARNING] Invalid cost matrix detected in Hungarian matcher.")
            matched_row_inds = torch.zeros(0, dtype=torch.long)
            matched_col_inds = torch.zeros(0, dtype=torch.long)
...
class OCGroupNorm(nn.Module):
    ...
    def forward(self, x):
        N, C = x.shape[:2]  # 배치 크기, 채널 개수
        G = self.num_groups

        assert C % G == 0, f"num_channels {C} should be divisible by num_groups {G}"

        # 입력 데이터를 (N, G, C // G, *) 형태로 변환
        original_shape = x.shape  # 원래 shape 저장
        x = x.view(N, G, C // G, -1)  # (N, G, C//G, *)으로 변환
        ...
        x = x.view(original_shape)  # 원래 형태로 복원

        # Affine 변환 적용 (가중치 & 편향)
        if self.affine:
            weight = self.weight.view(1, C, *([1] * (x.dim() - 2)))
            bias = self.bias.view(1, C, *([1] * (x.dim() - 2)))
            x = x * weight + bias
