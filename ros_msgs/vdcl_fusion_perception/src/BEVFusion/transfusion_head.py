...
class TransFusionHead(nn.Module):
    ...
    def forward_single(self, inputs, metas=None):
        ...
        # fusion_feat_heatmap = fusion_feat.clone()
        # fusion_feat = fusion_feat[2:3]
        ...
        with torch.autocast('cuda', enabled=False):
            # dense_heatmap = self.heatmap_head(fusion_feat_heatmap.float())
            self.heatmap_head = self.heatmap_head.float()
        ...
        if self.vis:
            ...
            name_list = ['cam', 'lidar', 'fusion']        
            for i in range(len(heatmap)):
                fig, ax = plt.subplots(1, 1, figsize=(6, 6))
                heatmap_vis = heatmap[i, :].clone().sum(axis=0).detach().cpu().numpy()
                im = ax.imshow(heatmap_vis , cmap='viridis')
                ax.set_title('Heatmap '+str(i))
                plt.savefig(f'test_data/heatmap_{img_num}_{name_list[i]}.png')
                plt.close()
        
        # dense_heatmap = dense_heatmap[2:3]
        # heatmap = heatmap[2:3]
        ...
        # local_max[:, :8, padding:(-padding), padding:(-padding)] = F.max_pool2d(
        #     heatmap[:, :8], kernel_size=self.nms_kernel_size, stride=(1,1), padding=0)
        # local_max[:, 8, ] = F.max_pool2d(
        #     heatmap[:, 8], kernel_size=1, stride=(1,1), padding=0)
        # local_max[:, 9, ] = F.max_pool2d(
        #     heatmap[:, 9], kernel_size=1, stride=(1,1), padding=0)
        heatmap_c01_7 = heatmap[:, :8, ...]    # 0~7번째 클래스
        heatmap_c8    = heatmap[:, 8:9, ...]   # 8번째 클래스
        heatmap_c9    = heatmap[:, 9:10, ...]  # 9번째 클래스

        # 2) 채널마다 다른 kernel_size 로 maxpool
        out_c01_7 = nn.MaxPool2d(kernel_size=self.nms_kernel_size, stride=1, 
                                padding=self.nms_kernel_size // 2)(
                                    heatmap_c01_7
                                )
        out_c8    = nn.MaxPool2d(kernel_size=1, stride=1, padding=0)(heatmap_c8)
        out_c9    = nn.MaxPool2d(kernel_size=1, stride=1, padding=0)(heatmap_c9)

        # 3) 다시 합침
        local_max = torch.cat([out_c01_7, out_c8, out_c9], dim=1)
        ...
        # for i in range(1):
        #     fig, ax = plt.subplots(1, 1, figsize=(6, 6))
        #     heatmap_vis = heatmap[i, :].clone().view(-1,256,256).sum(axis=0).detach().cpu().numpy()
        #     # heatmap_vis = heatmap[i, :].clone().view(-1,180,180).sum(axis=0).detach().cpu().numpy()
        #     im = ax.imshow(heatmap_vis , cmap='viridis')
        #     fig.suptitle(f'{name_list[i]} Heatmap')
        #     plt.savefig(f'test_data/heatmap_maxpool_{img_num}_{name_list[i]}.png')
        #     plt.close()
        ...
        # im = ax.imshow(heatmap_vis , cmap='viridis')
        # bbox_num = 400
        # for bn in range(bbox_num):
        #     center = ret_dicts[0]['center'][0, :, bn].detach().cpu().numpy()
        #     width = ret_dicts[0]['dim'][0, 0, bn].detach().cpu().numpy()
        #     length = ret_dicts[0]['dim'][0, 1, bn].detach().cpu().numpy()
        #     rot = ret_dicts[0]['rot'][0, 0, bn].detach().cpu().numpy()
        #     x = center[0]
        #     y = center[1]
        #     width = width
        #     length = length
        ...
        #     y1 = y - length / 2
        #     y2 = y + length / 2
        #     rect = plt.Rectangle((x1, y1), width, length, linewidth=1, edgecolor='r', facecolor='none')
        #     ax.add_patch(rect)
        # ax.plot
        # fig.suptitle(f'{name_list[i]} Heatmap')
        # plt.savefig(f'test_data/heatmap_{img_num}_{name_list[i]}.png')
    ...
    def get_targets(self, batch_gt_instances_3d: List[InstanceData],
        ...
        for batch_idx in range(len(batch_gt_instances_3d)):
            ...
            for key in preds_dict[0].keys():
                preds = []
                preds.append(preds_dict[0][key][batch_idx:batch_idx+1,:,:])
                # for i in range(self.num_decoder_layers):
                #     print("key", key, "len(preds_dict[0][key])", preds_dict[0][key].shape)
                #     if not key in ['query_heatmap_score', 'dense_heatmap']:
                #         pred_one_layer = preds_dict[0][key][batch_idx:batch_idx+1,:,i*self.num_proposals:(i+1)*self.num_proposals]
                #     else:
                #         pred_one_layer = preds_dict[0][key][batch_idx:batch_idx+1,:,:]
                #     print("pred_one_layer", pred_one_layer.shape)
    ...
    def get_targets_single(self, gt_instances_3d, preds_dict, batch_idx):
        ...
        # self.gt_nums.append(len(gt_labels_3d))
        ...
        # self.pos_nums.append(len(pos_inds))
        # print('gt_nums:', np.max(self.gt_nums), np.min(self.gt_nums),
        #       np.mean(self.gt_nums))
        # print('pos_nums:', np.max(self.pos_nums), np.min(self.pos_nums),
        #         np.mean(self.pos_nums))
