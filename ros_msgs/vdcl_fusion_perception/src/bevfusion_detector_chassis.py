...
class LiDARSweeps:
    def __init__(self, num_sweeps=10):
        ...
        self.ts = deque(maxlen=num_sweeps)
        self.l2e = deque(maxlen=num_sweeps)
        self.vx = deque(maxlen=num_sweeps)
        self.yaw_rate = deque(maxlen=num_sweeps)

    def add(self, pts, l2e, vx, yaw_rate, stamp):
        self.points.append(pts.float())
        self.l2e.append(torch.from_numpy(l2e).cuda().float() if l2e is not None else None)
        self.vx.append(vx)
    ...
    def build(self):
        ...
        for i in reversed(range(len(points) - 1)):
            dt_total = t_cur - ts[i]
            if dt_total > 0.6 or l2es[i] is None:
                continue
            ...
            # (1) LiDAR → Ego 변환
            ones = torch.ones((pts.shape[0], 1), device='cuda', dtype=torch.float32)
            pts_h = torch.cat([pts[:, :3], ones], 1)
            pts_ego = (pts_h @ l2es[i].T)

            # (2) Dead Reckoning 누적 transform (과거 → 현재)
            T_total = np.eye(4, dtype=np.float32)
            for j in range(i, len(points) - 1):
                dt = ts[j+1] - ts[j]
                T_step = self.build_relative_transform(vxs[j], yaw_rates[j], dt)
                T_total = T_total @ T_step  # forward accumulate

            # (3) Transform 적용
            T_total_torch = torch.from_numpy(T_total).to(device='cuda', dtype=torch.float32)
            T_total_torch = torch.linalg.inv(T_total_torch)
            pts_warped = pts_ego @ T_total_torch.T

            pts_cur = pts_warped @ (torch.linalg.inv(l2e_cur).T)[:,:3]

            dt_tensor = torch.full((pts_cur.shape[0], 1), t_cur - ts[i], device='cuda', dtype=torch.float32)

            combined.append(torch.cat([pts_cur, inten, dt_tensor], 1))

        return [torch.cat(combined, 0).half()]
    
    def build_relative_transform(self, vx, yaw_rate, dt):
        dx = vx * dt
        dtheta = yaw_rate * dt
...
class CamImages:
    def __init__(self, cam_keys, shape=(256,704)):
        self.shape=shape
        self.buffers={k:TimeBuffer(20) for k in cam_keys}
        self.cam_keys=cam_keys
    ...
    def fetch(self, target,max_dt=0.2):
        ...
        for k in self.cam_keys:
            ...
            if im is None:
                ...
                # im = torch.from_numpy(im).cuda().permute(2,0,1)
                t,a=self._prep(im)
                imgs.append(t)
                augs.append(a)        
...
class InferenceNode:
    def __init__(self):
        ...
        self.lidar=LiDARSweeps(num_sweeps=1)
        self.motion_buffer = TimeBuffer()

        self.work_q=deque(maxlen=1)
        ...
        rospy.Subscriber('ego_twist', TwistStamped, self._ego_twist_cb, queue_size=1)
    ...
    def _img_cb(self,msg,cam):
        ...
        # self.cams.add(img,cam,msg.header.stamp.to_sec())
    ...
    def _ego_twist_cb(self, msg):
        stamp=msg.header.stamp.to_sec()
        t=[msg.twist.linear.x,msg.twist.angular.z]
        self.motion_buffer.add(stamp,t)

    def _lidar_cb(self,msg):
        stamp=msg.header.stamp.to_sec()
        pts=self._pc2_to_xyzi(msg)
        # e2g,_=self.ego_tf.get_closest(stamp,0.1)
        motion = self.motion_buffer.get_closest(stamp,0.1)[0]
        l2e,_=self.lidar_tf.get_closest(stamp,0.1)
        if l2e is None:
            rospy.logwarn(f"[Skip] No lidar-to-ego TF at {stamp}")
            return
        self.lidar.add(pts,l2e,motion[0],motion[1],stamp)
        # extrinsics 최신화
        for cam in CAM_KEYS:
            mat,_=self.cam_tf[cam].get_closest(stamp,0.1)
            if mat is not None:
                self.meta.cam_extrinsics[cam]=mat
        self.meta.lidar_extrinsics=l2e
        # stamp 큐 push (최신만 유지)
        # while not self.work_q.empty():
        #     try: self.work_q.get_nowait()
        #     except queue.Empty: break
        self.work_q.append(stamp)

    # ---------------- Worker ----------------
    def _worker(self):
        while not rospy.is_shutdown():
            start_time = time.time()
            if len(self.work_q) == 0:
                continue
            stamp=self.work_q.pop()
            self._infer(stamp)
